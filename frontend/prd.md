...
Every API-backed screen handles unavailable data and stable `VALIDATION_ERROR`/location errors gracefully; protected routes require the correct role; a refreshed URL reproduces search state; and both Vite and Next.js production builds complete in CI. The Vite build must remain within both configured JavaScript budgets; the measured initial entry after route splitting is 213.0 KiB. Root `npm run test:e2e` must pass the Chromium traveler booking/cancellation/refund, supplier assignment acceptance, operations task/support/refund, and Mopa service-area validation journeys.
