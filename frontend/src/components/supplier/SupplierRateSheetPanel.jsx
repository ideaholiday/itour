...
const NEW_SERVICE = () => ({ kind: "SIGHTSEEING", name: "", city: "", fromPlace: "", toPlace: "", closedWeekdays: [], dayTitle: "", dayDescription: "" });
 ...
 * (ADR 042). Cars are priced per vehicle for each cab type, activities per
 * adult and child, by season. Used only to price quotations; never listed.
 */
export default function SupplierRateSheetPanel({ supplierId, onChange }) {
  ...
  const addRate = (service) => (event) => {
    ...
    const body = isTransport(service.kind)
      ? { cabTypeId: draft.cabTypeId || cabTypes[0]?.id, validFrom: draft.validFrom, validTo: draft.validTo, vehicleInr: Number(draft.vehicleInr) }
          ...
          return (
            <div key={service.id} className="rounded-2xl border border-stone-200 p-4">
              <p className="font-bold text-stone-900">{service.name}
                <span className="font-normal text-stone-500"> · {SERVICE_KIND_LABELS[service.kind]}{service.city ? ` · ${service.city}` : ""}{service.fromPlace || service.toPlace ? ` · ${service.fromPlace || "?"} → ${service.toPlace || "?"}` : ""}{service.closedWeekdays.length ? ` · closed ${service.closedWeekdays.map((day) => WEEKDAYS[day]).join(", ")}` : ""}</span>
              </p>
              {service.dayTitle && <p className="text-xs text-stone-500">Day title: {service.dayTitle}</p>}
              <table className="mt-2 w-full text-left text-xs">
                <thead className="text-[10px] uppercase text-stone-400"><tr><th className="py-1">{car ? "Cab" : "Ticket"}</th><th>Season</th><th className="text-right">{car ? "Per vehicle" : "Adult"}</th>{!car && <th className="text-right">Child</th>}<th /></tr></thead>
                <tbody className="divide-y divide-stone-100">
                  {service.rates.map((rate) => (
                    <tr key={rate.id}>
                      ...
                      <td className="text-right font-mono">{inr(car ? rate.vehicleInr : rate.adultInr)}</td>{!car && <td className="text-right font-mono">{inr(rate.childInr)}</td>}
