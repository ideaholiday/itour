...
export default function SupplierListingsPanel({ products = [], supplierId, onRefresh }) {
  ...
  const handleCopyLink = (product) => {
    ...
    const origin = window.location.origin || "https://ideaholiday.in";
    const fullUrl = `${origin}${activityPath(product)}`;
  ...
  return (
    <section id="supplier-listings" className="scroll-mt-28 rounded-3xl border border-stone-200 bg-white p-5 sm:p-6 shadow-sm">
      ...
      {inventoryProduct && <SupplierInventoryEditor supplierId={supplierId} product={inventoryProduct} onClose={() => setInventoryProduct(null)} />}
      {/* Messages */}
      {error && (
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 font-mono text-[10px] bg-amber-50 px-1.5 py-0.5 rounded border border-amber-300 text-amber-900 hover:bg-amber-100 transition"
                    title="Copy SEO Public Link"
                  >
                    {copiedLink === product.id ? <Check className="w-2.5 h-2.5 text-emerald-600" /> : <ExternalLink className="w-2.5 h-2.5 text-amber-700" />}
                    <span>{copiedLink === product.id ? "Link Copied!" : "Copy SEO Link"}</span>
                  </button>
                  <span
              ...
              <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                ...
                {product.product_type !== "TRANSFER" && <button type="button" onClick={() => setInventoryProduct(product)} className="rounded-lg border border-emerald-700 px-3 py-2 text-xs font-semibold text-emerald-800">Seats and schedule</button>}

                <Link
                  to={activityPath(product)}
                  target="_blank"
                  rel="noreferrer"
                  title={live ? "View live on marketplace" : "Preview marketplace listing"}
