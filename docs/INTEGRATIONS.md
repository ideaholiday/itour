...
- `POST /api/octo/v1/bookings/reservation`: 10-minute temporary seat hold (`native_reservations`). This and the other `bookings` endpoints need a partner key (`Authorization: Bearer ihp_…`, issued by `scripts/create-api-partner.js`, or by a supplier's owner for their own resellers in the extranet, ADR 041); see API_CONTRACTS §6.
