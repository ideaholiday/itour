...
### 3.10 Hotel rate sheet, cars and activities, and package quotations (ADR 040, ADR 042–044)
Rules: [`SUPPLIER_OPERATIONS.md`](SUPPLIER_OPERATIONS.md). Owner or manager.
...
- **`POST .../hotels/:hotelId/rates`** `{ roomType, mealPlan: EP|CP|MAP|AP, validFrom, validTo, netPerNightInr, extraAdultInr?, childInr?, maxGuests? }` → `201 { hotel }`; `409 RATE_OVERLAP`. **`DELETE .../hotels/:hotelId/rates/:rateId`** → `{ hotel }`.
- **`GET .../cab-types`**, **`POST .../cab-types`** `{ name, seats, status? }`, **`PUT .../cab-types/:cabTypeId`** → `{ cabType: { id, name, seats, status } }` (list: `{ cabTypes }`).
- **`GET .../services`**, **`POST .../services`**, **`PUT .../services/:serviceId`** `{ kind: TRANSFER|SIGHTSEEING|ACTIVITY, name, city?, fromPlace?, toPlace?, startTime?, durationHours?, closedWeekdays?: [0–6, 0 = Sunday], dayTitle?, dayDescription?, status?, pricing?: FIXED|PER_KM, distanceKm? }` → `{ service: { …, rates: [{ id, cabTypeId, validFrom, validTo, vehicleInr, adultInr, childInr, perKmInr, minKmPerDay, driverAllowanceInr }] } }` (list: `{ services }`). `409 KIND_CHANGE` when a car service would become an activity or back; `409 PRICING_CHANGE` when switching fixed ↔ per km while it has seasons.
- **`POST .../services/:serviceId/rates`** `{ cabTypeId, validFrom, validTo, vehicleInr }` for a fixed-price car, `{ cabTypeId, validFrom, validTo, perKmInr, minKmPerDay?, driverAllowanceInr? }` for a per-km car, `{ validFrom, validTo, adultInr, childInr? }` for an activity → `201 { service }`. Errors: `400 CAB_TYPE_REQUIRED`, `400 PRICE_REQUIRED`, `404 CAB_TYPE_NOT_FOUND`, `409 RATE_OVERLAP`. **`DELETE .../services/:serviceId/rates/:rateId`** → `{ service }`.
- **`GET .../package-library`** → `{ regions, items: [{ id, region, kind, name, city, fromPlace, toPlace, distanceKm, durationHours, dayTitle, dayDescription, sedanInr, innovaInr, tempoInr, adultInr, childInr, added }] }` (ADR 047; live entries only). **`POST .../package-library/import`** `{ itemIds: [1–100] }` → `201 { added: [names], skipped: [names already added], cabTypesAdded: [names], validFrom, validTo }`: one service per entry with a season from today for a year at the example prices; all or nothing, `404 LIBRARY_ITEM_NOT_FOUND` for an unknown or hidden entry. Admin: **`GET /api/admin/package-library`** → `{ regions, items }` (hidden included); **`POST /api/admin/package-library`**, **`PUT /api/admin/package-library/:id`** `{ region, kind, name, city?, fromPlace?, toPlace?, distanceKm?, durationHours?, dayTitle?, dayDescription?, sedanInr?, innovaInr?, tempoInr?, adultInr?, childInr?, sortOrder?, status? }` → `{ item }`; `400 PRICE_REQUIRED` for a car with no cab price or an activity with no adult price.
- **`GET .../route-library`** (ADR 048) → `{ routes: [{ id, own, region, name, description, legs: [{ city, nights }], nights, days: [{ dayNumber, title, description, itemIds, items: [{ id, name, kind, serviceId }] }], inclusions, exclusions, status }], cities: [{ id, region, name, description, dayTitle, dayDescription }] }`: the supplier's own routes first, then the live shared ones; `serviceId` is the supplier's service added from that library entry, or null. **`POST .../routes`**, **`PUT .../routes/:routeId`** `{ name, region?, description?, legs: [1–20 { city, nights }], days?: [{ dayNumber, title?, description?, itemIds? }], inclusions?, exclusions? }` → `{ route }`; `400 DAY_OUTSIDE_ROUTE` (a day after nights + 1), `DUPLICATE_DAY`, `LIBRARY_ITEM_NOT_FOUND`; `404 ROUTE_NOT_FOUND` for a shared or another supplier's route. **`DELETE .../routes/:routeId`** → `{ deleted }` (own routes only). Admin: **`GET /api/admin/route-library`** → `{ routes, cities }` (shared, hidden included); **`POST`/`PUT /api/admin/route-library/routes[/:id]`** (same body, plus `sortOrder?`, `status?`) → `{ route }`; **`POST`/`PUT /api/admin/route-library/cities[/:id]`** `{ region, name, description?, dayTitle?, dayDescription?, sortOrder?, status? }` → `{ city }`, `409 CITY_EXISTS`.
- **`GET /api/suppliers/:id/quotations`** → `{ quotations: [{ id, ref, title, destination, customerName, agentId, agentName, startDate, status, totalInr, updatedAt }] }`. A quotation with an agent also carries `agentName` and `trade: { markupPct, netInr, marginInr, options: [{ number, netInr, marginInr }] }` (ADR 048): the agent's net at its own markup, the same figure as the trade PDF and the send-to-agent email.
- **`GET .../quotation-terms`** → `{ terms: { inclusions, exclusions } }` (empty lists until saved). **`PUT .../quotation-terms`** `{ inclusions?: string[], exclusions?: string[] }` (up to 40 items of 1–300 characters each) → `{ terms }`: the standard lists, replaced whole (migration 077).
- **`GET .../quotations/suggestions?destination=&days=`** → `{ quotations: [{ id, ref, title, destination, days, adults, children, status, totalInr, updatedAt }] }`: up to 8 past quotations whose destination contains the text and, when `days` is given, with that many days.
- **`POST .../quotations`** / **`PUT .../quotations/:quotationId`** `{ title, destination?, customerName, customerEmail?, customerPhone?, agentId?, startDate, adults, children, markupPct, notes?, inclusions?: string[], exclusions?: string[], validUntil?, lines: [HOTEL { dayNumber, option?, title, description?, hotelId, roomType, mealPlan, checkIn, nights, rooms, extraAdults, children } | LISTING { dayNumber, title, description?, productId, productOptionId?, date, pickupTime?, adults, children } | CUSTOM { dayNumber, title, description?, date?, amountInr } | TRANSPORT { dayNumber, title?, description?, serviceId, cabTypeId, date, vehicles?, km?, carDays?, adults?, children? } | ACTIVITY { dayNumber, title?, description?, serviceId, date, adults?, children? }], days?: [{ dayNumber, title?, description? }], options?: [{ name }] }` → `{ quotation: { id, ref, title, destination, status, customer…, lines: [… priceInr, serviceId, cabTypeId, vehicles, bookingId], days, totals: { costInr, listingsInr, markupInr, subtotalInr, gstPct, gstInr, totalInr, paidInr, dueInr, perPersonInr }, options: [{ number, name, totals: { costInr, listingsInr, markupInr, subtotalInr, gstInr, totalInr, perPersonInr } }], selectedOption, payments, warnings: [string] } }`. Hotel options (ADR 043): 2–6 `options` make hotel lines belong to `option` (1-based, default 1) and every other line shared; `totals` are option 1's until one is accepted; `options` is `[]` for a single package; `400 UNKNOWN_OPTION` for a hotel in a missing option. Every line is priced on the server; a client price is never read. A rate-sheet line without `title` takes the service's name, and without counts the quotation's travelers; a transport line without `vehicles` gets enough cabs for everyone. A day without text takes the first rate-sheet service's day title and description. Errors: `404 HOTEL_NOT_FOUND`, `404 PRODUCT_NOT_FOUND`, `404 AGENT_NOT_FOUND`, `404 SERVICE_NOT_FOUND`, `404 CAB_TYPE_NOT_FOUND`, `409 RATE_MISSING`, `400 KM_REQUIRED` (per-km car with no km and no usual distance), `409 SERVICE_CLOSED`, `409 WRONG_SERVICE_KIND`, `409 LISTING_UNAVAILABLE`, `409 QUOTATION_FINAL` (accepted or declined).
- **`POST .../quotations/:quotationId/copy`** `{ startDate, customerName?, customerEmail?, customerPhone? }` → `201 { quotation }`: a new `DRAFT` with every date moved to the new start and priced again; the save errors apply (`409 RATE_MISSING` past a season's end).
...
- **`POST .../quotations/:quotationId/status`** `{ status: SENT|ACCEPTED|DECLINED, option? }` → `{ quotation }`; `409 INVALID_TRANSITION` (only draft → sent → accepted or declined). Accepting a quotation with hotel options needs `option`, whose totals become the quotation's (`400 OPTION_REQUIRED`).
