...
### 3.10 Hotel rate sheet, cars and activities, and package quotations (ADR 040, ADR 042, ADR 043)
Rules: [`SUPPLIER_OPERATIONS.md`](SUPPLIER_OPERATIONS.md). Owner or manager.
...
- **`GET .../services`**, **`POST .../services`**, **`PUT .../services/:serviceId`** `{ kind: TRANSFER|SIGHTSEEING|ACTIVITY, name, city?, fromPlace?, toPlace?, startTime?, durationHours?, closedWeekdays?: [0–6, 0 = Sunday], dayTitle?, dayDescription?, status? }` → `{ service: { …, rates: [{ id, cabTypeId, validFrom, validTo, vehicleInr, adultInr, childInr }] } }` (list: `{ services }`). `409 KIND_CHANGE` when a car service would become an activity or back.
- **`POST .../services/:serviceId/rates`** `{ cabTypeId, validFrom, validTo, vehicleInr }` for a transfer or sightseeing, `{ validFrom, validTo, adultInr, childInr? }` for an activity → `201 { service }`. Errors: `400 CAB_TYPE_REQUIRED`, `400 PRICE_REQUIRED`, `404 CAB_TYPE_NOT_FOUND`, `409 RATE_OVERLAP`. **`DELETE .../services/:serviceId/rates/:rateId`** → `{ service }`.
