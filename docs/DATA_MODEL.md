...
- **`quotations`** (migration 067): `ref` (`Q-XXXXXX`), `title`, customer name/email/phone, optional `agent_id`, `start_date`, `adults`, `children`, `markup_pct`, `notes`, `valid_until`, `status` (`DRAFT`, `SENT`, `ACCEPTED`, `DECLINED`) and the server's totals: `cost_inr` (hotel and custom lines), `listings_inr`, `markup_inr`, `subtotal_inr`, `gst_pct`, `gst_inr`, `total_inr`.
- **`quotation_lines`** (migration 067): `kind` `HOTEL` (hotel, room, meal plan, check-in `line_date`, `nights`, `rooms`, `extra_adults`, `children`), `LISTING` (`product_id`, option, `line_date`, `pickup_time`, `adults`, `children`, and `booking_id` once booked) or `CUSTOM` (`amount_inr`); `day_number`, `title`, `description` and `price_inr`, the server-worked cost or pre-tax price.
