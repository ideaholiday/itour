...
- **`supplier_hotels`** / **`supplier_hotel_rates`** (migration 067, ADR 040): the supplier's hotel rate sheet, used only to price quotations. A rate is `room_type`, `meal_plan` (`EP`, `CP`, `MAP`, `AP`), a season `valid_from`–`valid_to` (seasons for the same room and meal plan never overlap), `net_per_night_inr`, `extra_adult_inr`, `child_inr`.
- **`supplier_cab_types`** (`name`, `seats`), **`supplier_services`**, **`supplier_service_rates`** (migration 069, ADR 042): private car and activity prices. Service `kind` `TRANSFER`/`SIGHTSEEING`/`ACTIVITY`, `closed_weekdays` (0 = Sunday), `day_title`/`day_description`. Rate: season + `cab_type_id` and `vehicle_inr`, or `adult_inr` and `child_inr`.
- **`quotations`** (migration 067; `destination` in 069): `ref` (`Q-XXXXXX`), `title`, `destination`, customer name/email/phone, optional `agent_id`, `start_date`, `adults`, `children`, `markup_pct`, `notes`, `valid_until`, `status` (`DRAFT`, `SENT`, `ACCEPTED`, `DECLINED`) and the server's totals: `cost_inr` (hotel, transport, activity and custom lines), `listings_inr`, `markup_inr`, `subtotal_inr`, `gst_pct`, `gst_inr`, `total_inr`.
- **`quotation_lines`** (migration 067): `kind` `HOTEL` (hotel, room, meal plan, check-in `line_date`, `nights`, `rooms`, `extra_adults`, `children`), `LISTING` (`product_id`, option, `line_date`, `pickup_time`, `adults`, `children`, and `booking_id` once booked) `CUSTOM` (`amount_inr`), `TRANSPORT` (`service_id`, `cab_type_id`, `vehicles`) or `ACTIVITY` (`service_id`) (069); `day_number`, `title`, `description` and `price_inr`, the server-worked cost or pre-tax price. 069 drops the `kind` CHECK; the service validates it.
- **`quotation_days`** (069): `day_number`, `title`, `description`.
- **`quotation_payments`** (migration 067): money the customer paid for an accepted package: `amount_inr`, `mode`, `reference`, `received_by`.
