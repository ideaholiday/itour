...
- **`SUPPLIER`**: Can only access listings, drivers, assignments, and payouts belonging to their verified `supplier_id`. The owner is the user whose email matches `suppliers.email`. **Staff** (ADR 036) are users linked in `supplier_members` as `MANAGER`, `FRONT_DESK` or `GUIDE`; the principal carries `supplier_role`. `supplierRoleAllows` (`supplierStaffService.js`) runs after the supplier scope check on `/api/suppliers/:id/*`: managers are refused KYB, payout, subscription, plans, spotlights, business profile and staff; front desk and guides are refused anything not on their allowlist (`403 SUPPLIER_ROLE_FORBIDDEN`). Reviews, support cases, enquiries and the channel manager need owner or manager; KYB uploads need the owner. Non-owners never receive bank details, PAN, KYB documents or payouts from `GET /api/suppliers/:id`; guides get no booking lists and no balances on the manifest. A guide linked to a guide resource sees and checks in only departures they are assigned to (ADR 037). Front desk may list agents to book for them but not change agents, rates or payments on account (ADR 039). Only the owner manages staff; temporary passwords are returned once and stored as scrypt hashes.
...
- **OCTo partners** (`api_partners`): machine callers, not users. A supplier's owner may issue and revoke keys for their own resellers in the extranet (`issuer = 'SUPPLIER'`, ADR 041); those keys reach only that supplier's listings with the own-reseller channel on, and never carry the `prepaid` flag. Booking endpoints under `/octo` need `Authorization: Bearer ihp_…`; only the key's SHA-256 is stored, the key is printed once by `scripts/create-api-partner.js` and revoked with `--revoke`. A partner reaches only its own OCTo reservations, never marketplace holds, and only a `prepaid` partner's confirmation is recorded as paid.
