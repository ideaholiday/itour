...
- Supplier side (`/api/suppliers/:id/subscription`, the supplier or `ADMIN`/`STAFF`): **`GET`** → `{ subscription, plan: { priceInr, billingPeriodMonths, gstRatePct } | null, payments }`; **`POST /quote`** `{ couponCode? }` → `{ quote: { periodMonths, baseInr, coupon, discountInr, taxableInr, gstRate, gstInr, totalInr, sacCode } }` (`409 NOT_FOR_SALE` / `NOT_REQUIRED`); **`POST /checkout`** `{ couponCode?, returnUrl? }` → `201 { quote, payment, checkout: { orderId, paymentSessionId, environment } | null }` (`null` for a 100% coupon; `502 PAYMENT_UNAVAILABLE`); **`POST /payments/:paymentId/verify`** → `{ status, invoiceNumber, subscription }` (`409 NOT_PAID`, `AMOUNT_MISMATCH`); **`GET /payments/:paymentId/invoice`** → HTML tax invoice. The Cashfree webhook (`/api/checkout/cashfree/webhook`) also activates `subs_…` orders.
- `supplier_subscriptions` settings also take `priceInr` (rupees before GST, `null` = not for sale) and `billingPeriodMonths` (1–36, default 12). Coupons take `audience: "TRAVELER" | "SUPPLIER_SUBSCRIPTION"` on create (fixed after).
