...
2. **Who sees it.** Operations, the booking's supplier and the traveler see the driver's position for the whole trip. The traveler's "driver on the way" and "trip started" messages carry a private tracking link (`/track/<ref>`, valid 7 days); signed-in travelers use Track live in My Trips. The traveler sees the driver, vehicle, position and ETA (to pickup while on the way, to drop-off during the trip), never before the driver accepts and not after completion.
6. **Arrival prompt.** Within 150 m of a pickup that has map coordinates, the driver page asks the driver to tap Arrived.
7. **Missed-pickup alerts.** From 30 minutes before pickup until the pickup-not-started alert takes over, a `DRIVER_LOCATION_RISK` task (HIGH) and one supplier and operations alert per reason are raised when the driver has not gone on the way, has lost signal on the way, is estimated more than 10 minutes late, or has not moved for 10 minutes more than 1 km from pickup. A lost signal also asks the driver to reopen the trip page. Arriving, starting or completing closes the task.
...
5. **Web limits and the app.** In a browser the trip page only sends while it is open on screen; it keeps the screen awake and sends at least once a minute. Locked phones or switching to another app stop updates, which shows as Delayed or Signal lost. In the Idea Holiday Driver Android app a foreground location service keeps sharing (with an ongoing notification) while the driver uses Maps or the phone is locked, until the trip is completed or the server ends it.
