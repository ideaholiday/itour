...
- **Consequences**: Still open: whether our Cashfree SecureID plan includes driving licence and vehicle RC checks (until confirmed, those documents go to manual review); a D1 per-country licence rule; GST on non-GST owners' cab bookings (section 9(5)); Europe payouts and VAT/DAC7/GDPR. Japan, and China if any Chinese city uses a different zone, must be checked against the fixed-offset time code; Japan has no daylight saving.
