...
| Package quotations and hotel rate sheet, PDF and share link (ADR 040) | `suppliers.js` (`/:id/hotels`, `/:id/quotations`), `quotations.js` (public `/share/:token`) | `quotationService`, `quotationPdfService` (pdfkit), `supplierHotelService`, `supplierBookingService` (package-line bookings), `emailService` (attachments) | `components/supplier/SupplierQuotationsPanel.jsx` (`?panel=packages`), `components/supplier/SupplierHotelRatesPanel.jsx` |
