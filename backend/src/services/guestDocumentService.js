...
export function renderGuestDocument(documentType, booking) {
  ...
  const charges = Math.min(total + friendDiscount + walletCredit, Number(booking.tolls_and_tax_amount || 0));
  const serviceValue = Math.max(0, total + friendDiscount + walletCredit - charges);
