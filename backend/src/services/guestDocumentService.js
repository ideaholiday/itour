...
export function renderGuestDocument(documentType, booking) {
  ...
  if (type === "VOUCHER") {
    ...
    const body = `<h1>Confirmed booking voucher</h1><p class="muted">Present this mobile voucher at pickup. Government-issued identification may be requested.</p><div class="grid"><div class="card"><span class="label">Experience</span><strong>${escapeHtml(booking.product_title || booking.product_type)}</strong></div><div class="card"><span class="label">Traveler</span><strong>${escapeHtml(booking.traveler_name)}</strong><br>${escapeHtml(booking.traveler_phone)}</div><div class="card"><span class="label">Date and time</span><strong>${escapeHtml(booking.activity_date)} · ${escapeHtml(booking.pickup_time || "TBC")}</strong></div><div class="card"><span class="label">Operator</span><strong>${escapeHtml(booking.supplier_name || "Idea Holiday partner")}</strong><br>${escapeHtml(booking.supplier_phone || "")}</div></div><section class="section"><h2>Pickup / meeting point</h2><div class="card">${escapeHtml(booking.pickup_location)}${booking.pickup_instructions ? `<br><span class="muted">${escapeHtml(booking.pickup_instructions)}</span>` : ""}</div></section>${booking.drop_location ? `<section class="section"><h2>Drop-off</h2><div class="card">${escapeHtml(booking.drop_location)}</div></section>` : ""}<section class="section"><h2>Driver and vehicle</h2><div class="card">${driver}</div></section><div class="notice"><strong>Pickup security:</strong> Check the driver and vehicle plate before sharing the private pickup code shown only in My Trips. The code is intentionally excluded from this shareable voucher.</div>`;
