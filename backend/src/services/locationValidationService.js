...
function explicitRules(db, productId) {
  try {
    ...
    WHERE r.product_id = ? AND (r.is_active IS NULL OR CAST(r.is_active AS TEXT) NOT IN ('0', 'false'))
    `).all(productId);
  } catch {
...
export function validatePickupPoint(db, productId, side, userLat, userLng, userAddress = "") {
  ...
  if (mode === "FIXED_LOCATION") {
    let fixed = rule.fixed_location_id
      ? db.prepare("SELECT * FROM canonical_locations WHERE id = ? AND (is_active IS NULL OR CAST(is_active AS TEXT) NOT IN ('0', 'false'))").get(rule.fixed_location_id)
      : null;
