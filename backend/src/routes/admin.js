...
router.get("/metrics", optionalAuthMiddleware, requireAdminAccess, (req, res) => {
  try {
    ...
    const geoZones = db.prepare("SELECT COUNT(*) as count FROM geo_fences WHERE is_active = TRUE").get().count;
    const coveredCities = db.prepare("SELECT COUNT(DISTINCT city) as count FROM geo_fences WHERE is_active = TRUE").get().count;
  ...
  try {
    ...
    const cityCoverage = db.prepare(`
      SELECT d.id, d.name, d.state,
             COUNT(CASE WHEN gf.is_active = TRUE AND COALESCE(gf.approval_status, 'APPROVED') = 'APPROVED' THEN 1 END) AS active_zones,
             COUNT(DISTINCT CASE WHEN gf.is_active = TRUE AND COALESCE(gf.approval_status, 'APPROVED') = 'APPROVED' THEN gf.supplier_id END) AS active_suppliers
      FROM destinations d
