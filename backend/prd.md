...
- Expose APIs for activities, places, transfers, availability, authentication, bookings, checkout, suppliers, operations, administration, analytics, support, reviews, and notification webhooks.
- Support 5 operational product types (`TRANSFER`, `DAY_TOUR`, `MULTI_DAY_PACKAGE`, `ATTRACTION_TICKET`, `EXPERIENCE`) and persist specialized options in additive tables: `product_ticket_tiers` (Adult/Child/Senior/Infant), `product_vehicle_options`, `product_sic_hubs`, `product_hotel_tiers`, and `product_itinerary_items`.
- Implement Native Reservations Phase 1 Engine (`native_inventory_rules`, `native_availability_slots`, `native_reservations`) with transaction-serialized capacity mutations on the product row.
- Support owner-bound 10-minute temporary seat reservation holds (`POST /api/availability/native/hold`), calculate remaining vacancies dynamically (`vacancies = capacity - confirmed - active_holds`), and release expired or failed holds immediately.
- Enforce supplier-configured cut-off rules (`cutoff_minutes`) and blackout dates without cancelling existing confirmed bookings.
- Provide instant booking confirmation for native inventory options upon verified payment, bypassing the manual 24-hour supplier acceptance timer and issuing QR vouchers immediately.
- Enqueue confirmed dispatches into `native_reservation_outbox` and run a durable background processor with retries, leases, and replay-safe idempotency keys.
- Map external identities via `reservation_external_references` through an abstracted OCTo-aligned boundary (`reservationProviders.js`).
...
- Support dual SQLite & PostgreSQL versioned database migrations (`_schema_migrations`) with forward batch application and rollback capabilities across migrations `001_initial_schema.sql` through `020_supplier_channel_connections.sql`.
...
- Support multi-channel ResTech integration endpoints (`/api/supplier-channels`) allowing suppliers to connect external booking systems (Bókun, FareHarbor, Bookingkit, TourCMS, Activitar, Anchor, generic OCTo), test connection health, fetch remote products, and import catalog items with `reservation_external_references` mappings.
- Implement the OCTo specification engine (`/api/octo/v1/*` and `/octo/v1/*`) exposing capabilities, supplier metadata, product catalog, real-time availability, reservation holds, confirmations, and cancellations.
- Enforce portal-scoped login verification on `/api/auth/login` (`portal=admin` requires `ADMIN`/`STAFF`; `portal=supplier` requires `SUPPLIER`).

## Acceptance Criteria

Protected APIs require `Authorization: Bearer <express-or-supabase-token>`. API errors use `{ error, code, requestId }`, responses include `X-Request-Id`, and validation failures use `400/VALIDATION_ERROR` without echoing input. Location failures additionally include a safe `detail.suggestion` and allowed area/state. `GET /api/metrics` requires `ADMIN`/`STAFF` or `X-Metrics-Token`; `POST /api/telemetry/web-vitals` rejects unknown and unbounded fields. Local tests run deterministically without live providers or developer-database copies. `cd backend && npm run test:coverage` must pass all 264 backend unit tests across 9 test suites and the 70% line/function gate, and `npm run test:integration` must complete the 13 real-HTTP traveler, native reservation hold, circuit-order, grouped-payment, and location-validation journeys before merge. Versioned schema changes and migrations execute atomically via `npm run migrate:up/status/down`, support SQLite-safe conditional columns, verify applied-file checksums, and never remove ledger entries without a real down migration. Destructive seed and migration operations require explicit intent.
