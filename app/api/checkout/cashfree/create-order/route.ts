...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/api/checkout/cashfree/create-order`, {
      method: 'POST',
      headers: {
    ...
    console.error('Next.js Cashfree Create Order Proxy Error:', error);
    return NextResponse.json(
      { error: (error as Error).message || 'Failed to initialize Cashfree order' },
      { status: 502 }
    );
  }
}
