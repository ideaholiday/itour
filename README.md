...
```bash
cd backend && npm test              # 266 unit tests across 9 suites
cd backend && npm run test:integration  # 14 real-HTTP journey tests
...
- `.github/workflows/ci.yml`: Runs 266 backend unit tests across 9 suites, 14 HTTP integration tests, coverage gates, Playwright E2E journeys, and builds on PR/push.
...
├── backend/
│   ├── migrations/                  # Versioned SQL Migration Files (001 to 020)
│   ├── src/
│   │   ├── engine/
...
│   ├── test/                        # 266 Deterministic Backend Unit Tests (9 Suites)
│   └── integration/                 # 14 Real-HTTP Isolated Journey Tests
├── frontend/
│   ├── src/
│   │   ├── pages/
│   │   │   ├── admin/
